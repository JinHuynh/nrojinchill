...
//    public static final int SEND_NOTIFY = 514;
    public static final int QUY_DOI_COIN = 508;
    public static final int BTV = 513;
    ...
    public static final int DOI_COINT_SANG_VND = 520;

    public static final int TAI = 510;
    public static final int XIU = 511;
    public static final int TOTAL = 561;
                    ...
                    String username = text[0].trim();
                        int addvnd = Integer.parseInt(text[1].trim());
                           if (Client.gI().getPlayerByUserName(username) != null) {
                               PlayerDAO.Addvnd(username, addvnd);
                               PlayerDAO.Addtotalvnd(username, addvnd);
                               Service.getInstance().sendThongBao(player, "Bạn đã buff  VNĐ và TỔNG VNĐ cho " + username + " " + addvnd + "VNĐ");
                           Client.gI().getPlayerByUserName(username).getSession().vnd += addvnd;
                           
                            Service.getInstance().sendThongBao(Client.gI().getPlayerByUserName(username), "Bạn vừa được cộng " + addvnd + " bới " + player.name);
                           } else {
                               Service.gI().sendThongBao(player, "Không tìm thấy player : "+ username);
                           }
                }
                break;
                case GIVE_IT:
...
//                case SEND_NOTIFY:
//                    if (player.isAdmin()) {
//                    Service.getInstance().sendThongBao(player, "Thông báo cho cư dân thành công");
//                    } else {
//                        Service.getInstance().sendThongBao(player, "Thông báo cho cư dân Thất bại");
//                    }
//                    break;
                case SEND_ITEM_OP:
                    if (player.isAdmin()) {
...
//                    Service.getInstance().doiRubySangPet(player, text[0]);
//                    break;
//                }
                  case GIFT_CODE:{
                ...
                case CHOOSE_LEVEL_BDKB:
                    ...
                    if (level >= 1 && level <= 110) {
                        ...
                        if (npc != null) {
                            ...
                            ChangeMapService.gI().changeMap(player, 135, -1, 318, 336);
                            break;
    ...
    public void createFormDoiTVSangRuby(Player pl) {
        try {
            createForm(pl, DOI_VANG_SANG_RUBY,
                    ...
                    new SubInput("Số thỏi vàng muốn đổi", NUMERIC));
    ...
    public void createFormDoiRubySangPet(Player pl) {
        try {
            createForm(pl, DOI_RUBY_SANG_PET,
                    ...
                    new SubInput("Số hộp pet muốn đổi", NUMERIC));
    ...
    public void createFormNapThe(Player pl, byte loaiThe) {
      ...
      public void createFormBuffVNDANDTOTALVND(Player player) {
        createForm(player, TOTAL, "Buff VNĐ và tổng nạp",
                new SubInput("Tài Khoản người chơi", ANY),
                new SubInput("Sô tiền cần buff", ANY));
    }
      
    public void TAI(Player pl) {
        createForm(pl, TAI, "Chọn số thỏi vàng đặt Xỉu", new SubInput("Số thỏi vàng", NUMERIC));// ????
    }

    public void XIU(Player pl) {
        createForm(pl, XIU, "Chọn số thỏi vàng đặt Tài", new SubInput("Số thỏi vàng", NUMERIC));
    }
...
//    public void createFormSendThongBao(Player pl) {
//        createForm(pl, SEND_NOTIFY, "SEND Thông báo",
//                new SubInput("Thông báo", ANY));
//    }

    public void createFormQDTV(Player pl) {

        createForm(pl, QUY_DOI_COIN, "Quy đổi thỏi vàng, giới hạn đổi không quá 500 "
                + "\n10.000 Vnd = 10 Thỏi vàng "
                + "\n20.000 Vnd = 20 Thỏi vàng "
                + "\n50.000 Vnd = 50 Thỏi vàng "
                + "\n100.000 Vnd = 100 Thỏi vàng "
                + "\n200.000 Vnd = 200 Thỏi vàng "
                + "\n500.000 Vnd = 500 Thỏi vàng", new SubInput("Nhập số lượng muốn đổi", NUMERIC));
    }
    public void createFormBTV(Player pl) {

        createForm(pl, BTV, "Quy đổi thỏi vàng, giới hạn đổi không quá 500 "
                + "\nx1 = 500tr vàng "
                + "\nx5 = 2,5tỉ vàng "
                + "\nx10 = 5tỉ vàng "
                + "\nx15 = 7,5tỉ vàng "
                + "\nx20 = 10tỉ vàng "
                + "\nx25 = 12,5tỉ vàng", new SubInput("Nhập số lượng muốn đổi", NUMERIC));
    }
//    public void createFormBTV(Player pl) {
//        createForm(pl, BTV, "Bán thỏi vàng", new SubInput("Nhập số lượng muốn bán", NUMERIC));
//    }
