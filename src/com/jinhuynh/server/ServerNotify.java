    ...
    public void run() {
        while (!Maintenance.isRuning) {
            try {
                ...
                if (Util.canDoWithTime(this.lastTimeGK, 30000)) {
                    sendThongBaoBenDuoi("Game không có bản quyền, cân nhắc trước khi chơi (nrojinchill.online).");
                    this.lastTimeGK = System.currentTimeMillis();
                }
                if (Util.canDoWithTime(this.lastTimeGK, 30000)) {
                    sendThongBaoAllPlayer("|7|Admin ĐANG VÀO\n" + "|1|...\n...\n...\n"+ "|2|MẤY THẰNG HACK, BUG CẨN THẬN BAND...");
                    this.lastTimeGK = System.currentTimeMillis();
                }
