...
import com.jinhuynh.utils.FileIO;
import com.jinhuynh.data.DataGame;
import com.jinhuynh.jdbc.daos.GodGK;
...
import com.jinhuynh.models.matches.PVP;
import com.jinhuynh.models.matches.PVPManager;
import com.jinhuynh.models.matches.TOP;
import com.jinhuynh.models.player.Player;
import com.jinhuynh.models.shop.ItemShop;
import com.jinhuynh.models.shop.Shop;
...
import static com.jinhuynh.services.func.SummonDragon.DRAGON_SHENRON;
...
public class Service {
    ...
    public static void sendInfoEffChar(Player pl, short id, byte layer, int loop, short loopCount, byte isStand) {
        Message msg;
        try {
            msg = new Message(-128);
            ...
            msg.writer().writeByte(layer); // 0 hin sau nv - 1 hiện trước nv
    ...
    public void chat(Player player, String text) {
        ...
        if (player != null) {
            if (player.getSession() != null && player.isAdmin()) {
                ...
                if (text.equals("eff")) {
                    Service.gI().sendInfoEffChar(player, (short) 58, (byte) 1, -1, (short) 10, (byte) -1);
                    return;
                }
...
//                }   
                ...
                if (text.equals("killpl")) {
                    ...
                    for (Player pl : playersMap) {
                        if (pl != null && !player.equals(pl)) {
                            ...
                            Service.getInstance().sendThongBao(pl, "|7|ADMIN ĐÃ TÀN SÁT CẢ SERVER");
                        }
                    }
                    return;
                }
                if (text.equals("thongbao")) {
                    this.sendThongBao(player, "Đã thông báo cho toàn cư dân");
                    List<Player> playersMap = Client.gI().getPlayers();
                    for (Player pl : playersMap) {
                        if (pl != null && !player.equals(pl)) {
                            Service.getInstance().sendThongBaoOK(pl, "Loa loa loa: Admin đẹp troai...");
                        }
                    }
                    return;
                }
                if (text.equals("admin")) {
                    NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_ADMIN, -1,
                            "|7|Builds server: Jin Huỳnh \n" + "|8|Admin: Lồn Hoàng" + "\n" + "|1|Số người online: " + Client.gI().getPlayers().size() + "\n",
                            //                                    + "|3|Current thread: " + Thread.activeCount() + "\n",
                            "Ngọc rồng", "Đệ tử", "Bảo trì", "Tìm kiếm\nngười chơi", "Boss", "VND + Tổng Nạp", "Đóng");
                    return;

                } 
//                else if (text.equals("notify")) {
//                    Input.gI().createFormSendThongBao(player);
//                } 
                    ...
                    // Tách tin nhắn thành các thành phần sử dụng khoảng trắng làm dấu phân cách
                    ...
                    if (parts.length >= 3) {
                        try {
                            ...
                            // Tạo số lượng sản phẩm tương ứng và thêm vào túi đồ của người chơi
                            for (int i = 0; i < quantity; i++) {
                                ...
                                // Thông báo cho người chơi về việc nhận sản phẩm
                                Service.gI().sendThongBao(player, "GET " + item.template.name + " [" + item.template.id + "] SUCCESS !");
                            }

                            InventoryServiceNew.gI().sendItemBags(player);
                        } catch (NumberFormatException e) {
                            // Xử lý nếu mã số sản phẩm hoặc số lượng không hợp lệ
                            Service.gI().sendThongBao(player, "Tin nhắn không hợp lệ. Sử dụng định dạng 'i {itemID} {số lượng}'.");
                        }
                    }
                } //            else if (text.startsWith("i ")) {
...
// JINCHILL FIX
        if (player != null && player.pet != null) {
            ...
            // Xử lý trường hợp player hoặc player.pet là null
    ...
    public void player(Player pl) {
        ...
        try {
            ...
            msg.writer().writeShort(7); //idauraeff
...
//     }
    ...
    public void attackMob(Player pl, int mobId) {
        if (pl != null && pl.zone != null) {
            for (Mob mob : pl.zone.mobs) {
                if (mob.id == mobId) {
                    SkillService.gI().useSkill(pl, null, mob, _msg);
                    break;
                }
            }
        }
    ...
    public void attackPlayer(Player pl, int idPlAnPem) {
        SkillService.gI().useSkill(pl, pl.zone.getPlayerInMap(idPlAnPem), null, _msg);
    }
...
//            int tongso = sl * 40;
//            Item thoivang = ItemService.gI().createNewItem((short) 2033, sl);
    ...
    public void showInfoPet(Player pl) {
        if (pl != null && pl.pet != null) {
            ...
            try {
                ...
                msg.writer().writeByte(4); // counnt pet skill // số lượng skill pet
