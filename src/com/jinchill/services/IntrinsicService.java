...
//    }
    public void open(Player player) {
    long requiredPower = 10000000000L;
    
    if (player.nPoint.power >= requiredPower) {
        Service.gI().sendThongBao(player, "Chỉ có thể mở nội tại bằng hồng ngọc.");
    } else {
        Service.gI().sendThongBao(player, "Yêu cầu sức mạnh tối thiểu 10 tỷ");
    }
}
    ...
    public void openVip(Player player) {
        if (player.nPoint.power >= 10000000000L) {
            ...
            if (player.inventory.ruby >= 500) {
                player.inventory.ruby -= gemRequire;
                PlayerService.gI().sendInfoHpMpMoney(player);
                changeIntrinsic(player);
                player.playerIntrinsic.countOpen = 0;
            } else {
                Service.gI().sendThongBao(player, "Bạn không có hồng ngọc, còn thiếu "
                        + (gemRequire - player.inventory.ruby) + " hồng ngọc nữa");
            }
        } else {
            Service.gI().sendThongBao(player, "Yêu cầu sức mạnh tối thiểu 10 tỷ");
        }
